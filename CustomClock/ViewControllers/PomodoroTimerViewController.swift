...
class PomodoroTimerViewController: UIViewController {
    ...
    private func setUpView() {
        let leftButton = UIBarButtonItem(title: "戻る", style: UIBarButtonItem.Style.plain, target: self, action: #selector(PomodoroTimerViewController.goBackButton))
        self.navigationItem.leftBarButtonItem = leftButton
        
        let rightButton = UIBarButtonItem(title: "終了", style: UIBarButtonItem.Style.plain, target: self, action: #selector(PomodoroTimerViewController.finishButton))
        ...
        workTime = selectArray[0]
        restTime = selectArray[1]
        repeatCount = selectArray[2]
    }
    ...
    private func workStart() {
        ...
        print(elapsedTime)
        workTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(PomodoroTimerViewController.workHandleTimer), userInfo: nil, repeats: true)
    }
    ...
    @objc private func workHandleTimer() {
        ...
        if elapsedTime == 0 {
            ...
            restTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(PomodoroTimerViewController.restHandleTimer), userInfo: nil, repeats: true)
        }
